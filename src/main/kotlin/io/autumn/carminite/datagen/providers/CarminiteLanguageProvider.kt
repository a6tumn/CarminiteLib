...
abstract class CarminiteLanguageProvider(
    packOutput: FabricPackOutput,
    languageCode: String,
    registryLookup: CompletableFuture<HolderLookup.Provider>
): FabricLanguageProvider(packOutput, languageCode, registryLookup) {
    abstract fun generateCarminiteTranslations(registryLookup: HolderLookup.Provider, translationBuilder: CarminiteTranslationBuilder)

    final override fun generateTranslations(
        registryLookup: HolderLookup.Provider,
        translationBuilder: TranslationBuilder
    ) {
        val wrapper = object : CarminiteTranslationBuilder {
            override fun add(translationKey: String, value: String) {
                translationBuilder.add(translationKey, value)
            }
        }
        generateCarminiteTranslations(registryLookup, wrapper)
    }

    @ApiStatus.NonExtendable
    interface CarminiteTranslationBuilder: TranslationBuilder {
        fun addToolSet(toolSet: ToolSet) {
        ...
        fun addWoodSet(woodSet: WoodSet, chestBlock: Block, trappedChestBlock: Block) {
            ...
            add(chestBlock, "${woodSet.woodName} Chest")
            add(trappedChestBlock, "Trapped ${woodSet.woodName} Chest")
        }
    }
}